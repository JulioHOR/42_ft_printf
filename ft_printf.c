...
/*   Updated: 2025/01/14 20:38:31 by juhenriq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_last_tfmt_spec_node(t_input *tinput_str, t_fmt_spec *tfmt_spec)
{
...
{
	const char	*string;
	int			fmt_specifier_idx;
	t_fmt_spec	*tfmt_spec;

	string = tinput_str->input_str;
	fmt_specifier_idx = find_char(string, '%', 0);
	while (fmt_specifier_idx != -1)
	{
		tfmt_spec = create_fmt_specifier(fmt_specifier_idx);
		if (!(tfmt_spec))
			return (1);
		if (determine_data_type(tfmt_spec, string, fmt_specifier_idx + 1))
			return (1);
		if (determine_flags(tfmt_spec, string, fmt_specifier_idx))
			return (1);
		add_last_tfmt_spec_node(tinput_str, tfmt_spec);
		if (tfmt_spec->data_type == TYPE_PERCENT_SIGN)
			fmt_specifier_idx = find_char(string, '%', fmt_specifier_idx + 2);
		else
			fmt_specifier_idx = find_char(string, '%', fmt_specifier_idx + 1);
	}
	return (0);
}
